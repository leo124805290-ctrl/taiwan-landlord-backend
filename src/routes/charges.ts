...
router.post('/generate', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 4. 過濾出用戶選擇的項目
    const chargesToGenerate = suggestedCharges.filter(charge => 
      chargeTypes.includes(charge.type)
    );
    ...
    for (const charge of chargesToGenerate) {
      ...
      const existingQuery = await client.query(
        `SELECT id FROM payments 
         WHERE tenant_id = $1 AND type = $2 AND status = 'pending' AND archived = FALSE`,
        [tenant_id, charge.type]
      );
      ...
      if (charge.type === 'deposit' && tenant.deposit_status === 'pending') {
        await client.query(
          'UPDATE tenants SET deposit_status = $1 WHERE id = $2',
          ['paid', tenant.id]
        );
      }
    }
    ...
    // 廣播更新
    broadcast('payments_updated', { tenant_id, payments: generatedPayments });
 ...
 * 計算單個租客的建議繳費項目
 */
async function calculateSuggestedCharges(client: any, tenant: any): Promise<any[]> {
  const charges = [];
  ...
  while (monthCursor <= currentMonth) {
    ...
    // 檢查是否已經有這個月的租金記錄
    const existingRentQuery = await client.query(
      `SELECT id FROM payments 
       WHERE tenant_id = $1 AND type = 'rent' AND month = $2 AND archived = FALSE`,
      [tenant.id, monthStr]
    );
...
router.get('/summary', async (req: Request, res: Response) => {
  try {
    const [tenantsResult, paymentsResult] = await Promise.all([
      pool.query("SELECT COUNT(*) as count, SUM(rent_amount) as total_rent, SUM(deposit_amount) as total_deposit FROM tenants WHERE status = 'active'"),
      pool.query("SELECT COUNT(*) as count, SUM(total_amount) as total_amount FROM payments WHERE status = 'pending' AND archived = FALSE")
    ]);
