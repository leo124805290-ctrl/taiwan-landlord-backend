...
router.post('/', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [property_id, floor, room_number, monthly_rent, deposit, status, tenant_name, check_in_date, check_out_date, current_meter, previous_meter]
    );
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [floor, room_number, monthly_rent, deposit, status, tenant_name, check_in_date, check_out_date, current_meter, previous_meter, id]
    );
